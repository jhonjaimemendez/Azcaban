...
final public class RotacionProveedor extends CrearJFrame implements ActionListener, FocusListener {
   ...
   //Columnas y filas estaticas 
    private Object [] nombresColumnas = {"Nit","Razón Social","Cantidad","Total"};
    private Object [][] datos = new Object[16][4];
    ...
    //** Constructor General 
    public RotacionProveedor(ConectarMySQL p_conectarMySQL,JFrame p_frame) {

      super("Rotación de Proveedores","Toolbar",p_frame);
      ...
      jpVentas = getJPanel("Proveedores",60, 130, 700, 380,14);
	...
	private void configurarColumnas() {
		...
		tablaVentasProducto.getColumnModel().getColumn(2).setPreferredWidth(210);
  ...
  //*****************************Metodo traerInformacion() ******************************************
    ...
    final private boolean traerInformacion() { 
        ...
        if (jcPeriodos.getSelectedIndex() == 0)
            ...
            sentenciaSQL    =   "Select  P.Nit,P.RazonSocial,Count(1),Sum(C.total) "+
                                "From  ComprasEncabezado C,Proveedores P "+ 
                                "Where C.NumeroProveedor = P.NumeroProveedor and C.Estado = 'G' "+
                                "Group by P.NumeroProveedor,P.Nit,P.RazonSocial "+
                                "Order by 3 "+ orden +
          ...
          if (jcPeriodos.getSelectedIndex() == 1)
                ...
                sentenciaSQL    =    "Select  P.Nit,P.RazonSocial,Count(1),Sum(C.total) "+
                                     "From  ComprasEncabezado C,Proveedores P "+ 
                                     "Where C.NumeroProveedor = P.NumeroProveedor and C.Estado = 'G'   and  Substr(C.Fecha,1,10) = '" + getObtenerFecha(conectarMySQL) + "' "+
	                                 "Group by P.NumeroProveedor,P.Nit,P.RazonSocial "+
	                                 "Order by 3 "+ orden +
	                                 "Limit " + numeroRegistro ; 
		          ...
		          if (jcPeriodos.getSelectedIndex() == 2) {
		              ...
		              String condicion = ""; 
			           ...
			           sentenciaSQL    =     "Select  P.Nit,P.RazonSocial,Count(1),Sum(C.total) "+
                                             "From  ComprasEncabezado C,Proveedores P "+ 
                                             "Where C.NumeroProveedor = P.NumeroProveedor and C.Estado = 'G' and substr(C.Fecha,1,7) = '" + getObtenerFecha(conectarMySQL).substring(0,7) + "' " +
							                 "      and Substr(C.Fecha,1,10) " + condicion +
			                                 "Group by P.NumeroProveedor,P.Nit,P.RazonSocial "+
			                                 "Order by 3 "+ orden +
			                                 "Limit " + numeroRegistro ;                                
			          ...
			          if (jcPeriodos.getSelectedIndex() == 3) 
				          ...
				          if (jcPeriodos.getSelectedIndex() == 4) {
					           ...
					           sentenciaSQL    =    "Select  P.Nit,P.RazonSocial,Count(1),Sum(C.total) "+
                                                    "From  ComprasEncabezado C,Proveedores P "+ 
                                                   "Where C.NumeroProveedor = P.NumeroProveedor and C.Estado = 'G'  and  substr(C.Fecha,1,7) between " + condicion +
												    "Group by P.NumeroProveedor,P.Nit,P.RazonSocial "+
					                                "Order by 3 "+ orden +
					                                "Limit " + numeroRegistro ;                        
