...
final public class RotacionClientes extends CrearJFrame implements ActionListener, FocusListener {
   ...
   //Columnas y filas estaticas 
    private Object [] nombresColumnas = {"Tipo","Identificación","Nombres","Cantidad","Total"};
    ...
    //** Constructor General 
    public RotacionClientes(ConectarMySQL p_conectarMySQL,JFrame p_frame) {

      super("Rotación de Clientes","Toolbar",p_frame);
      ...
      jpVentas = getJPanel("Clientes",60, 130, 700, 380,14);
	  ...
	  JLabel jlMas = getJLabel("Mas vendidas",80, 25, 80, 20);
      jpCriterios.add(jlMas);
      
	  JLabel jlMenos = getJLabel("Menos vendidas:",330, 25, 100, 20);
	...
	private void configurarColumnas() {
		...
		tablaVentasProducto.getColumnModel().getColumn(0).setPreferredWidth(15);
		...
		tablaVentasProducto.getColumnModel().getColumn(2).setPreferredWidth(305);
		
		tablaVentasProducto.getColumnModel().getColumn(3).setPreferredWidth(28);
		tablaVentasProducto.getColumnModel().getColumn(3).setCellRenderer(getAlinearDerecha());
		
		
		tablaVentasProducto.getColumnModel().getColumn(4).setPreferredWidth(27);
    ...
    private void limpiar() { 
      ...
      jtNumeroMasVendidos.setText("10");
      jtNumeroMenosVendidos.setText("10");
  ...
  //*****************************Metodo traerInformacion() ******************************************
    ...
    final private boolean traerInformacion() { 
        ...
        if (jcPeriodos.getSelectedIndex() == 0)
            ...
            sentenciaSQL    =   "Select   Case C.TipoId WHEN 'C' THEN 'Cedula'"+ 
	                            "                     WHEN 'T' THEN 'Tarjeta Id.'"+
	                            "                     Else 'Pasaporte'"+
	                            "         End Tipo, C.idCliente,Concat(Nombre,Concat(' ',Apellido)) Nombres,Count(1),Sum(V.total) "+
                                "From  VentasEncabezado V,Clientes C "+ 
                                "Where V.IdCliente = C.idCliente and V.Estado = 'G' "+
                                "Group by C.idCliente, C.TipoId, C.Nombre,C.Apellido "+
                                "Order by 3 "+ orden +
          ...
          if (jcPeriodos.getSelectedIndex() == 1)
                ...
                sentenciaSQL    =    "Select   Case C.TipoId WHEN 'C' THEN 'Cedula'"+ 
	                                 ...
	                                 "         End Tipo, C.idCliente,Concat(Nombre,Concat(' ',Apellido)) Nombres,Count(1),Sum(V.total) "+
                                     "From  VentasEncabezado V,Clientes C "+ 
                                     "Where V.IdCliente = C.idCliente and V.Estado = 'G'  and  Substr(V.Fecha,1,10) = '" + getObtenerFecha(conectarMySQL) + "' "+
		          ...
		          if (jcPeriodos.getSelectedIndex() == 2) {
		              ...
		              String condicion = ""; 
			           ...
			           sentenciaSQL    =     "Select   Case C.TipoId WHEN 'C' THEN 'Cedula'"+ 
			                                 ...
			                                 "         End Tipo, C.idCliente,Concat(Nombre,Concat(' ',Apellido)) Nombres,Count(1),Sum(V.total) "+
		                                     "From  VentasEncabezado V,Clientes C "+ 
					                         "Where V.IdCliente = C.idCliente and V.Estado = 'G' and substr(V.Fecha,1,7) = '" + getObtenerFecha(conectarMySQL).substring(0,7) + "' " +
			          ...
			          if (jcPeriodos.getSelectedIndex() == 3) 
				          ...
				          if (jcPeriodos.getSelectedIndex() == 4) {
					           ...
					           sentenciaSQL    =    "Select   Case C.TipoId WHEN 'C' THEN 'Cedula'"+ 
			                                        ...
			                                        "         End Tipo, C.idCliente,Concat(Nombre,Concat(' ',Apellido)) Nombres,Count(1),Sum(V.total) "+
		                                            "From  VentasEncabezado V,Clientes C "+ 
					                                "Where V.IdCliente = C.idCliente and V.Estado = 'G' and  substr(V.Fecha,1,7) between " + condicion +
												    "Group by C.idCliente, C.Nombre,C.Apellido "+
					                                "Order by 3 "+ orden +
					                                "Limit " + numeroRegistro ;                        
